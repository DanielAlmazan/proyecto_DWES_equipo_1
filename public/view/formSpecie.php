...
<div class="container">
    ...
    <form action="<?= $action ?>" method="POST" enctype="multipart/form-data">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="scientificName" name="scientificName" required value="<?= isset($specie) ? $specie->getScientificName() : '' ?>">
        </div>

        <div class="mb-3">
            <label for="commonName" class="form-label">Nombre común:</label>
            <input type="text" class="form-control" id="commonName" name="commonName" required value="<?= isset($specie) ? $specie->getCommonName() : '' ?>">
        </div>

        <div class="mb-3">
            <label for="climate" class="form-label">Clima:</label>
            <input type="text" class="form-control" id="climate" name="climate" required value="<?= isset($specie) ? $specie->getClimate() : '' ?>">
        </div>

        <div class="mb-3">
            <label for="region" class="form-label">Región:</label>
            <input type="text" class="form-control" id="region" name="region" required value="<?= isset($specie) ? $specie->getRegion() : '' ?>">
        </div>

        <div class="mb-3">
            <label for="daysToGrow" class="form-label">Días para creecer:</label>
            <input type="number" class="form-control" id="daysToGrow" name="daysToGrow" required value="<?= isset($specie) ? $specie->getDaysToGrow() : '' ?>">
        </div>

        <div class="mb-3">
            <label for="benefits" class="form-label">Beneficios:</label>
            <textarea class="form-control" id="benefits" name="benefits" required><?= isset($specie) ? implode(',', $specie->getBenefits()) : '' ?></textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="url" name="url" required value="<?= isset($specie) ? $specie->getUrl() : '' ?>">
        </div>
        <br>
